                                                      ...
                                                      const
{
  ...
  if (tm.is_zero_datetime())
  {
    ...
    return field->store_time_dec(zero.get_mysql_time(), item->decimals);
  }
  return field->store_timestamp_dec(tm.tv(), item->decimals);
}
...
Timestamp_or_zero_datetime_native_null::
  Timestamp_or_zero_datetime_native_null(THD *thd, Item *item, bool conv)
   :Null_flag(false)
{
  DBUG_ASSERT(item->type_handler() == &type_handler_timestamp2 || conv);
  if (conv ?
